#!/bin/bash
# Build script to assemble and test GGB:
# Google Groups For Business microservice.
...
function makeConfigTar {
    ...
    #    print "generalize so not studentdash specific TEST"
    
    # if no files then just return
    if [ $# -eq 0 ]; then
        echo "no files for config tar"
        return
    fi
    ...
    (
        ...
        tar -c -f ../ARTIFACTS/configuration-files.$ts.tar "$@"
...
function makeWarFile {
    set -x
    local war_prefix=$1
#    echo "generalize so not studentdash specific TEST"
    set -e
    atStep "make war file"
    echo "++++++ making free standing executible war +++++++++"
    warble _${WARBLER_VERSION}_ executable war
    
    #    mv StudentDashboard.war StudentDashboard.$ts.war
    
    # if no war file then die
    if [ $# -eq 0 ]; then
        echo "no files for config tar"
        ls -l
        exit 1
    fi
    mv ${war_prefix}.war ${war_prefix}.$ts.war
    mv *.war ./ARTIFACTS
    set +x
}
...
#    echo "generalize so not studentdash specific"
    ...
    local TIMESTAMP_value=$(ls ARTIFACTS/${war_prefix}.*.war | perl -n -e 'm/.+\.(.+)\.war/ && print $1' )
    local WEBAPPNAME_value=${war_prefix}
    vars=`cat <<EOF
########################
...
## should be no op if no files supplied
makeConfigTar
